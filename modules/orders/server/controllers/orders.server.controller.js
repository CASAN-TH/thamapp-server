...
exports.startdate = function (req, res, next, enddate) {
  ...
  Order.find({ created: { $gte: startdate, $lte: end } }).sort('-created').where('deliverystatus').ne('cancel').populate('user').populate('items.product').populate('namedeliver').exec(function (err, orders) {
    if (err) {
      return next(err);
    } else if (!orders) {
      return res.status(404).send({
