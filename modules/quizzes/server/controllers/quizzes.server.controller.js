...
exports.list = function (req, res) {
  Quiz.find().sort('-created').populate('user', 'displayName').populate('quizs.answers.user').populate('users').exec(function (err, quizzes) {
    if (err) {
      return res.status(400).send({
        message: errorHandler.getErrorMessage(err)
      });
...
exports.quizByID = function (req, res, next, id) {
  ...
  Quiz.findById(id).populate('user', 'displayName').populate('users').populate('quizs.answers.user').exec(function (err, quiz) {
    if (err) {
      return next(err);
    } else if (!quiz) {
