...
exports.list = function(req, res) {
  Sell.find().sort('-created').populate('user', 'displayName').populate('client').exec(function(err, sells) {
    if (err) {
      return res.status(400).send({
        message: errorHandler.getErrorMessage(err)
      });
...
exports.sellByID = function(req, res, next, id) {
  ...
  Sell.findById(id).populate('user', 'displayName').populate('client').exec(function (err, sell) {
    if (err) {
      return next(err);
    } else if (!sell) {
