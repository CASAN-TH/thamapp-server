(function () {
  ...
  ReceiptsController.$inject = ['$scope', '$state', '$window', 'Authentication', 'receiptResolve', 'AccountchartsService'];

  function ReceiptsController($scope, $state, $window, Authentication, receipt, AccountchartsService) {
    var vm = this;
    ...
    function creditdayChanged(docdate) {
      ...
      // var noMath = (vm.receipt.drilldate - vm.receipt.docdate) / 86400000;
      // var parse = parseInt((vm.receipt.drilldate - vm.receipt.docdate) / 86400000);
      // vm.receipt.creditday = parse + 1;
