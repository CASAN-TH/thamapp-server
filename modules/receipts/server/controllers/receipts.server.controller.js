...
exports.read = function(req, res) {
  ...
  receipt.isCurrentUserOwner = req.user && receipt.user && receipt.user._id.toString() === req.user._id.toString();
...
exports.receiptByID = function(req, res, next, id) {
  ...
  Receipt.findById(id).populate('user', 'displayName').populate('client').populate('items.product').exec(function (err, receipt) {
    if (err) {
      return next(err);
    } else if (!receipt) {
